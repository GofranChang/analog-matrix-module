...
static struct gpio_dt_spec selects[] = {
    DT_FOREACH_PROP_ELEM_SEP(KSCAN_NODE, select_gpios, GPIO_DT_SPEC_GET_BY_IDX, (, ))};
...
static void calibration_cb(const struct zmk_analog_matrix_calibration_event *ev,
...
void clear_cb(const struct device *dev, struct zmk_analog_matrix_calibration_entry *entries,
...
static void disable_kscan(void *data) { kscan_disable_callback(kscan_dev); }
